...
class SimpleImage {
    ...
    function imagealphamask($picture) {
        ...
        /**
         * Save full alpha channel of the new image
         */
        imagesavealpha( $newPicture, true );
                        
        
        // Resize blend image 
        if( $xSize != imagesx( $picture->image ) || $ySize != imagesy( $picture->image ) ) {
            $tempPic = imagecreatetruecolor( $xSize, $ySize );
            imagecopyresampled( $tempPic, $picture->image, 0, 0, 0, 0, $xSize, $ySize, imagesx( $picture->image ), imagesy( $picture->image ) );
        ...
        for( $x = 0; $x < $xSize; $x++ ) {
            for( $y = 0; $y < $ySize; $y++ ) {
                ...
                $mask_color_index = imagecolorat( $mask, $x, $y );
                ...
                $blend_pixel = imagecolorallocatealpha($gd, $color[ 'red' ], $color[ 'green' ], $color[ 'blue' ], $color[ 'alpha' ]); 
                
                
                
                if($a != 127 && $r > 200){
                                        
                    imagesetpixel( $mask, $x, $y, $blend_pixel );
    ...
    function create_mask($fabric_image, $mask_image) {
        ...
        $fabric = imagecreatefrompng($fabric_image);
        
        $mask = imagecreatefrompng($mask_image);
           
        // Get sizes and set up new picture
        $xSize = imagesx( $fabric );
        
        $ySize = imagesy( $fabric );
        ...
        /**
         * Fill the newly created image $newPicture with the color [0, 0, 0, 127]
         */
        imagefill( $newPicture, 0, 0, imagecolorallocatealpha( $newPicture, 0, 0, 0, 127 ) );

        
        /**
         * Resize mask to have same size as base image
         */
        ...
        for( $x = 0; $x < $xSize; $x++ ) {
            for( $y = 0; $y < $ySize; $y++ ) {
                ...
                $mask_alpha = ($mask_color >> 24) & 0xFF; 
                                              
                $image_color = imagecolorat($fabric, $x, $y);
                
                $image_alpha = ($image_color >> 24) & 0xFF; 
                
                echo $mask_alpha.' ';
                                                              
                if(true){
                    
                    $alphacolorxy = imagecolorallocatealpha($newPicture, ($image_color >> 16) & 0xFF, ($image_color >> 8) & 0xFF, $image_color & 0xFF, $mask_alpha);
                    
                }
                else{
                    
                    $alphacolorxy = imagecolorallocatealpha($newPicture, ($image_color >> 16) & 0xFF, ($image_color >> 8) & 0xFF, $image_color & 0xFF, $image_alpha);
                    
                }
                
                imagesetpixel($newPicture, $x, $y, $alphacolorxy);
    ...
    function merge_images($picture){
        ...
        imagecopy($this->image, $picture->image, 0, 0, 0, 0, $xSize, $ySize);
    }
